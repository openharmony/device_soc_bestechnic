            ...
            if (g_gap_cb_func && g_gap_cb_func->aclStateChangedCallbak) {
                ...
                if (g_ibrt_enter_freeman_mode)
                {
                    ...
                    if (nvrecord_env) {
                        ...
                        nv_record_update_ibrt_info(STEREO_UNKNOW, &invaild_addr);
...
bool GetRemoteDevAddr(PairedDeviceInfo * devInfo)
{
    bt_bdaddr_t *remote_addr = NULL;
    if (devInfo == NULL) {
        return false;
    }
#ifdef SAVE_DEVICE_NAME
    remote_addr = app_bt_get_remote_device_address(BT_DEVICE_ID_1);
    if(remote_addr){
        memcpy_s(&(devInfo->addr),OHOS_BD_ADDR_LEN,(BdAddr *)remote_addr,OHOS_BD_ADDR_LEN);
        LOG_I("%s addr: ",__func__);
        DUMP8("%02x ",devInfo->addr.addr,BT_ADDR_OUTPUT_PRINT_NUM);
        return true;
    }
#endif
    return false;
...
{
    return &bt_gap_if_env_ctx;
}

void GapSetEnterFreemanModeFlag(bool freeman);
void GapSetEnterFreemanModeFlag(bool freeman)
{
    if (freeman)
    {
        bt_gap_if_env_ctx.ux_dev_mode = GAP_DEV_MODE_SINGLE;
    }
    else
    {
        bt_gap_if_env_ctx.ux_dev_mode = GAP_DEV_MODE_STEREO;
    }
    LOG_I("%s ux_mode= %d", __func__, bt_gap_if_env_ctx.ux_dev_mode);
}

void GapUxStereoStateChangeReport(StereoEvent status)
{
    LOG_I("%s status:%d", __func__, status);
    if (bt_gap_if_env_ctx.ux_stereo_state_change_cb)
    {
        bt_gap_if_env_ctx.ux_stereo_state_change_cb(status);
    }
}
