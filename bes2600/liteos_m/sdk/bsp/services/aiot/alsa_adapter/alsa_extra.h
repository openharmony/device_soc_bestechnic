...
typedef enum
{
    ALSA_AUDIO_FOCUS_USER_IDLE,
    ALSA_AUDIO_FOCUS_USER_WIFI,
    ALSA_AUDIO_FOCUS_USER_BT_A2DP,
    ALSA_AUDIO_FOCUS_USER_BT_SCO,
    ALSA_AUDIO_FOCUS_USER_BT_BIS,

    ALSA_AUDIO_FOCUS_USER_UNKNOWN,
} alsa_audio_focus_user_t;
...
extern "C"
{
    ...
    int alsa_audio_process_render_open(int sample_rate,
                                       int in_channels, int out_channels,
                                       int in_bits, int out_bits, uint32_t timeout);
    int alsa_audio_process_render_close(void);
    ...
    int alsa_audio_process_render_set_cfg_preset(uint8_t *cfg, uint32_t cfg_size);
    int alsa_audio_process_render_set_cfg_param(uint8_t *cfg, uint32_t cfg_size);
    int alsa_audio_process_render_set_type(uint8_t *cfg, uint32_t cfg_size);
    int alsa_audio_process_render_set_vol(uint8_t *cfg, uint32_t cfg_size);
    int alsa_audio_process_render_set_bass_tuning(uint8_t *cfg, uint32_t cfg_size);

    int alsa_audio_process_open(int sample_rate, int sw_ch_num, int hw_ch_num, int in_bits, int out_bits, uint32_t timeout);
    ...
    int alsa_set_playback_dma_buffer_size(uint32_t dma_buffer_size);

    int alsa_set_capture_dma_buffer_size(uint32_t dma_buffer_size);
    ...
    int alsa_set_speed(alsa_handle_t *h, float speed, float pitch, float rate);

    void pwm_debug_info_enable(uint8_t en);
